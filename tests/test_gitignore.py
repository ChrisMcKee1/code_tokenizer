...
def test_bypass_gitignore(test_project, tmp_path):
    ...
    assert processed_files == expected_files, f"Expected {expected_files}, but got {processed_files}"
    ...
    assert stats_bypass["stats"]["files_processed"] > stats_with_gitignore["stats"]["files_processed"]
    assert len(stats_bypass["successful_files"]) > len(stats_with_gitignore["successful_files"])

    # Verify specific patterns
    processed_paths = {os.path.relpath(f, str(test_project)).replace(os.sep, "/") for f in stats_bypass["successful_files"]}
