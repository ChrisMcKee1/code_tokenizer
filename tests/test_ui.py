...
class TestUI:
    ...
    def test_progress_display_comprehensive(self, console):
        ...
        stats = {
            ...
            "languages": {"Python": 5, "JavaScript": 3, "JSON": 2},
        }
        table = create_stats_table(stats)
        assert table is not None
    ...
    def test_error_display(self, console):
        ...
        # Test with no errors
        update_display(layout=layout, progress=progress, stats={}, status="Running")
        assert layout["footer"].renderable is not None
        ...
        errors = ["First error message", "Second error message", "Third error message"]
        update_display(
            layout=layout, progress=progress, stats={}, status="Multiple errors", errors=errors
        )
        footer_text = str(layout["footer"].renderable)
    ...
    def test_stats_table_formatting(self, console):
        ...
        # Test minimal stats
        minimal_stats = {"files_processed": 1, "total_tokens": 100}
